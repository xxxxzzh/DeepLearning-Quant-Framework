...
在这一阶段，系统完成了走向实盘前的最后也是最关键的一步：彻底清除“未来函数”，并实现了端到端的深度学习训练闭环，标志着 V1.0 稳定版正式定型。

### 1. 8维特征工程矩阵 (8D Feature Engineering)
* **动态特征工厂**：全面接入 `pandas_ta`，一键生成包含 MA5偏离度、MACD、RSI、ATR、成交量变化率等在内的 8 维特征矩阵。
* **自适应维度输入**：模型入口实现了自适应 (`feature_dim = features_norm.shape[1]`)，彻底告别特征数量硬编码，支持未来无缝扩展几十乃至上百个因子。

### 2. 彻底清除未来函数 (Future Function Elimination)
* **T+1 预测重构**：发现并修复了由于特征与目标未对齐导致的“上帝视角”数据泄露。引入严格的 `shift(-1)` 逻辑，确保模型使用 T 日特征预测 T+1 日收益，将 Rank IC 从“看图说话”的 0.71 修正至符合真实市场规律的 0.0846。
* **强制对齐液压钳**：独创 `min_len` 裁剪机制，完美解决特征与标签在时间序列平移后产生的 `IndexError: out of bounds` 维度错位问题。

### 3. 深度学习训练闭环与可复现性 (Training Loop & Reproducibility)
* **端到端训练框架**：纯手写 PyTorch `train_model` 循环，打通了从 `StockDataset` 到 `DataLoader` 再到 Transformer 模型的前向与反向传播流水线，并加入了安全的 Z-Score 标准化 (`np.nan_to_num`) 确保 Loss 稳健下降。
* **全局随机种子 (Global Seed)**：部署了跨 Numpy、PyTorch 和 CUDA 的底层随机种子锁 (`set_seed`)，告别“薛定谔的模型”，确保每一次回测的收益率、最大回撤等指标 100% 可复现。

### 4. V1.0 稳定版基准表现 (2023-2026 样本外)
| 策略类型 | 总收益 | 夏普比率 | 最大回撤 | Rank IC (真实) |
| :--- | :--- | :--- | :--- | :--- |
| **V1.0 保守型 (Absolute)** | **23.40%** | **0.48** | **19.54%** | **0.0846** |
| **V1.0 激进型 (Relative)** | 16.73% | 0.43 | 20.21% | - |

*结论：在去除了所有未来函数并锁死随机种子后，模型凭借多因子共振与 Transformer 注意力机制，依然在 3 年样本外取得了 23.40% 的稳健收益，且最大回撤控制在 20% 以内，正式达到了工业界可实盘运行的基础门槛。*

#  技术挑战与解决方案 (Technical Challenges)

在本项目从实验环境向工业级框架迁移的过程中，针对 Windows 环境下的深度学习适配及底层库冲突，成功解决了以下核心工程难题：

### 1. PyTorch 底层动态链接库加载异常 (WinError 127)
* **问题描述**：在执行 `import torch` 时，由于系统环境中的 C++ 运行时库版本混杂，导致 `shm.dll` 加载失败，引发 `OSError: [WinError 127] 找不到指定的程序`。
* **解决思路**：
    * 采用 `pip install --force-reinstall` 强制重写损坏的底层依赖文件。
    * **工程化修复**：在 `main.py` 入口处动态调用 `os.add_dll_directory`，强制将 Torch 核心库路径注入 Windows 搜索句柄。
* **经验总结**：这增强了系统在不同算力环境（尤其是 Windows Server）下的可移植性。

### 2. 多重数学加速库运行时冲突 (OpenMP Conflict)
* **问题描述**：由于系统同时调用了 `PyTorch` 与 `MKL/NumPy` 的加速算子，引发了 OpenMP 运行时的冗余加载冲突，导致程序崩溃。
* **解决思路**：
    * 通过环境变量 `os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"` 实现运行时的紧急容错处理。
    * **架构级优化**：在最新的 Month 8 重构版中，通过环境隔离与依赖版本锁定的方式，确保了大规模矩阵运算的稳定性。
* **经验总结**：在处理高性能计算（HPC）任务时，环境变量的精细化管理是确保系统高可用性的关键。

### 3. 数据计算中的数值陷阱 (NaN Handling)
* **问题描述**：在计算单序列数据的 Rank IC 时，由于样本量不足或预测值分布单一，常出现分母为零导致的 `NaN` 报错。
* **解决思路**：
    * 在 `utils.py` 中实现了鲁棒的 IC 计算逻辑，引入了对常数列的预校验及 `rolling` 窗口平滑处理。
* **经验总结**：量化系统的健壮性不仅取决于模型，更取决于对异常数据流的实时容错能力。
